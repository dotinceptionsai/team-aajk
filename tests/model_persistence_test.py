...
def test_should_convert_back_and_forth(tmp_path):
    ...
    pipeline.train(data_registry)

    persistence.save_pipeline(pipeline, tmp_path)

    loaded_model = persistence.load_pipeline(tmp_path, DummyFilterPipeline)
    ...
    assert loaded_model.files.train_id == ["train_id.txt"]
    assert loaded_model.files.train_ood == ["train_ood.txt"]
    assert loaded_model.files.validation_id == "validation_id.txt"
    assert loaded_model.files.validation_ood is None
